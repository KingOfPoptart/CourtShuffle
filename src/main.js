import './style.css';
import { AppState, Court } from './models';
    ...
    try {
        ...
        if (existingGym) {
            ...
            if (replace) {
                ...
                if (appState.activeGymId === existingGym.id)
                    appState.activeGymId = '';
            }
            else {
    ...
    <div class="gym-bar">
      ...
      ` : `
        <select class="gym-select" id="gym-select" aria-label="Select gym">
          ${appState.gyms.map((g) => `<option value="${g.id}"${g.id === appState.activeGymId ? ' selected' : ''}>${escapeHtml(g.name)}</option>`).join('')}
        </select>
            ...
            return `
              <li class="player-item${here ? ' player-present' : ''}">
                ...
                <select class="court-pref-select" data-pref-player="${p.id}"
                  ...
                  ${Array.from({ length: g.numCourts }, (_, i) => i + 1).map((n) => `<option value="${n}"${p.preferredCourt === n ? ' selected' : ''}>${escapeHtml(g.getCourtName(n))}</option>`).join('')}
                </select>
...
function renderCourtBody(court) {
    ...
    <ul class="court-players">
      ${court.players.map((p) => court.server?.id === p.id
...
function initDragAndDrop() {
    ...
    let draggedSourceCourtNum = null; // null = from bench
    let draggedSourceTeamIdx = null; // null unless from a team div
    ...
    function onPointerUp(e) {
        if (ghost)
            ghost.style.display = 'none';
        const targetItem = document.elementFromPoint(e.clientX, e.clientY)?.closest('.draggable-item');
        if (ghost)
            ghost.style.display = '';
        if (targetItem && draggedPlayerId && result) {
            ...
            if (targetId && targetId !== draggedPlayerId) {
                ...
                swapPlayers(draggedPlayerId, draggedSourceCourtNum, draggedSourceTeamIdx, targetId, targetCourtNum, targetTeamIdx);
                cleanup();
                return;
            }
        }
...
function movePlayer(playerId, sourceCourtNum, // null = bench
sourceTeamIdx, // null unless source was a team slot
targetZone) {
    if (!result)
        return;
    const el = targetZone;
    ...
    // No-op: same zone
    if (sourceCourtNum !== null && targetCourtNum === sourceCourtNum && targetTeamIdx === sourceTeamIdx)
        return;
    if (sourceCourtNum === null && targetIsBench)
        return;
    // ── Remove from source ───────────────────────────────────────────
    if (sourceCourtNum !== null) {
    ...
    if (targetCourtNum !== null) {
        ...
        if (!tgt)
            return;
        if (tgt.players.length >= Court.MAX_PLAYERS)
            return; // full
...
function swapPlayers(aId, aCourtNum, _aTeamIdx, bId, bCourtNum, _bTeamIdx) {
    if (!result)
        return;
    ...
    if (aCourtObj)
        swapInArray(aCourtObj.players);
    else
        swapInArray(result.bench);
    if (bCourtObj && bCourtObj !== aCourtObj)
        swapInArray(bCourtObj.players);
    else if (!bCourtObj && aCourtObj !== null)
        swapInArray(result.bench);
    // Teams arrays
    aCourtObj?.teams?.forEach(swapInArray);
    if (bCourtObj && bCourtObj !== aCourtObj)
        bCourtObj.teams?.forEach(swapInArray);
    [aCourtObj, bCourtObj].forEach((court) => {
        if (!court)
            return;
        if (court.server?.id === aId)
            court.server = bPlayer;
        else if (court.server?.id === bId)
            court.server = aPlayer;
    });
    render();
}
// ── Event wiring ──────────────────────────────────────────────────────────────
function wireEvents() {
    ...
    if (addingGym) {
        ...
        const renameInput = document.getElementById('gym-rename-input');
        renameInput.focus();
        renameInput.select();
        ...
        document.getElementById('gym-select').addEventListener('change', (e) => {
            appState.setActiveGym(e.target.value);
            result = null;
            save();
            render();
        });
    ...
    document.querySelectorAll('[data-toggle]').forEach((btn) => {
        btn.addEventListener('click', () => {
            gym().togglePresent(btn.dataset.toggle);
            result = null;
            save();
            render();
        });
    ...
    document.querySelectorAll('[data-remove]').forEach((btn) => {
        btn.addEventListener('click', () => {
            gym().removeFromPool(btn.dataset.remove);
            result = null;
            save();
            render();
        });
    ...
    document.querySelectorAll('.court-pref-select').forEach((sel) => {
        sel.addEventListener('change', (e) => {
            const playerId = sel.dataset.prefPlayer;
    ...
    document.querySelectorAll('.court-name-input').forEach((el) => {
        ...
        input.addEventListener('change', () => {
            ...
            document.querySelectorAll('.court-pref-select').forEach((sel) => {
                ...
                if (player)
                    s.value = player.preferredCourt?.toString() ?? '';
            });
        });
    });
    ...
    document.getElementById('clear-data-btn').addEventListener('click', () => {
        if (!confirm('Clear all gyms, players, and settings? This cannot be undone.'))
            return;
        localStorage.clear();
